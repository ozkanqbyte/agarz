...
const newCode = `  _handleEject(player, count = 1) {
    ...
    const ejected = []
    let ejectCount = 0
    outerLoop: for (const cell of player.cells) {
      while (ejectCount < count) {
        if (cell.mass <= EJECT_COST + 20) break outerLoop
        cell.mass -= EJECT_COST
        const dx = (player.inputX || 0) - cell.x
        const dy = (player.inputY || 0) - cell.y
        const d = Math.sqrt(dx * dx + dy * dy) || 1
...
if (content.includes(old)) {
  ...
  console.log('✓ Fixed eject - now ejects exactly count times total');
} else {
